...
/// <summary>
/// Validator for UpdateCurrencyPricingCommand
/// Enforces 0-9999 range and 50% deviation warnings (US5)
/// </summary>
public class UpdateCurrencyPricingValidator : AbstractValidator<UpdateCurrencyPricingCommand>
{
    private const decimal MinPrice = 0;
    private const decimal MaxPrice = 9999;
    private const decimal MaxDeviationPercent = 0.50m; // 50%

    public UpdateCurrencyPricingValidator()
    {
        RuleFor(x => x.Id)
            .NotEmpty().WithMessage("Currency pricing ID is required");
